...
class MTP_SPO2CART(object):
  ...
  def __init__(self, a,b,
    ...
    max_depth=float("inf")
    min_weights_per_node=0
    min_depth=None
    min_diff=0
    binary_splits=True
    debias_splits=False
    frac_debias_set=0.2
    min_debias_set_size=100
    quant_discret = 0.01
    ...
    ###############################################
    ###############################################
    #compute mapping from CART tree IDs to SPO node IDs
    CART2SPOinternal_node_ids = {}
    SPO_leaf_descendants = set()
    CART_id = 0
    for SPO_id in range(len(b)):
      if SPO_id in SPO_leaf_descendants:
         SPO_leaf_descendants.add(2*SPO_id+1)
         SPO_leaf_descendants.add(2*SPO_id+2)
      else:
        if is_leaf(a[:,SPO_id]):
          SPO_leaf_descendants.add(2*SPO_id+1)
          SPO_leaf_descendants.add(2*SPO_id+2)
        CART2SPOinternal_node_ids[CART_id] = SPO_id
        CART_id = CART_id + 1
    ...
    tree_params.a = a
    tree_params.b = b
    tree_params.CART2SPOinternal_node_ids = CART2SPOinternal_node_ids
  ...
  '''
  def fit(self, X, A, Y, weights=None, feats_continuous=True, verbose=False, refit_leaves=False, *leafargs_fit,**leafkwargs_fit):
...
def _find_best_split_binary(node, tree_params, leaf_params, verbose, X, A, Y, weights, sum_weights, feats_continuous, Xnames, shuffled_root_inds):
  ...
  if node.ind not in tree_params.CART2SPOinternal_node_ids:
    node.is_leaf = True
  ...
  a_node = tree_params.a[:,tree_params.CART2SPOinternal_node_ids[node.ind]]
  b_node = tree_params.b[tree_params.CART2SPOinternal_node_ids[node.ind]]
  if is_leaf(a_node):
    node.is_leaf = True
    del node.data_inds #we don't need this anymore, so delete for memory management
    return [node,None]
  
  node.split_var_ind = np.where(a_node == 1)[0][0]
  node.is_split_var_numeric = True
  node.split_val = b_node
